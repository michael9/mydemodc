...
public class AppFileDownUtils extends Thread {
	...
	private Message msg;
	private final String APP_FOLDER = "dice"; // sd卡应用目录
	private final String APK_FOLDER = "update"; // 下载apk文件目录
	...
	public void run() {
		try {
			if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
				...
				File folder = new File(sdcardDir + File.separator + APP_FOLDER + File.separator + APK_FOLDER);
				if (!folder.exists()) {
					folder.mkdirs();
				}
				File saveFilePath = new File(folder, mFileName);
				System.out.println(saveFilePath);
				mDownNotification = new Notification(android.R.drawable.stat_sys_download, mFileName, System.currentTimeMillis());
				...
				mContentView.setImageViewResource(R.id.downloadImage,android.R.drawable.stat_sys_download);
				...
				if (downSuc) {
					msg.what = MSG_FINISH;
					Notification notification = new Notification(android.R.drawable.stat_sys_download_done, mContext.getString(R.string.notification_download_success),System.currentTimeMillis());
					...
					notification.setLatestEventInfo(mContext, mContext.getString(R.string.notification_download_success), null,contentIntent);
					mNotifManager.notify(R.drawable.download, notification);
				} else {
					msg.what = MSG_FAILURE;
					Notification notification = new Notification(
					android.R.drawable.stat_sys_download_done, mContext.getString(R.string.notification_download_failure),System.currentTimeMillis());
					notification.flags = Notification.FLAG_AUTO_CANCEL;
					PendingIntent contentIntent = PendingIntent.getActivity(mContext, 0, new Intent(), 0);
					notification.setLatestEventInfo(mContext, mContext.getString(R.string.notification_download_failure), null, contentIntent);
					mNotifManager.notify(R.drawable.download, notification);
				}
			} else {
			...
			Log.e("error", "AppFileDownUtils catch Exception:", e);
			msg.what = MSG_FAILURE;
		} finally {
	...
	public boolean downloadFile(String downloadUrl, File saveFilePath) {
		...
		try {
				...
				while ((i = is.read(buffer)) != -1) {
					...
					synchronized (this) {
						if (downFileSize == fileSize) {
							// 下载完成
							mNotifManager.cancel(R.id.downloadImage);
							...
							mNotifManager.notify(R.id.downloadImage, mDownNotification);
			...
			result = false;
			Log.e("error", "downloadFile catch Exception:", e);
		}
		return result;
	}
}